...
@Script.Manifest(
        ...
        description = "v1.4f - Blast furnace (Steel, Mithril, Adamantite, Runite only)"
)
...
public class BlastFurnace extends PollingScript<ClientContext> implements PaintListener {
    private static String version = "1.4f";
...
//        graphics.fillRect(x, y, GUI_WIDTH - GUI_X, GUI_HEIGHT);
            ...
            if (Condition.wait(new Callable<Boolean>() { // wait until walking
                ...
                if (ctx.objects.peek().click("Put-ore-on", Game.Crosshair.ACTION) || ctx.objects.peek().interact(false, "Put-ore-on")) {
                    ...
                    Condition.wait(new Callable<Boolean>() {
                        @Override
                        public Boolean call() throws Exception {
                            log.info("Debug line 645");
                            return putOreWidget.valid();
                        }
                    }, 250, 10); // debug here
                    log.info("Debug line 649");
                } else {
    ...
    /**
     * @return number of bars collected
     */
    ...
    private boolean payForeman() {
        if (ctx.inventory.select().id(COIN_ID).peek().stackSize() >= PAYMENT_COST) {
            ...
            if (openBank()) {
                ...
                if(ctx.bank.withdraw(COIN_ID, Bank.Amount.ALL) && ctx.inventory.select().id(COIN_ID).poll().stackSize() < 2500) {
                    log.info("No more coins");
                    payFailCount++;
                    if(payFailCount > WITHDRAW_FAIL_THRESHOLD)
                        ctx.controller.stop();
                }
            }
        }
