...
public class BlastFurnace extends PollingScript<ClientContext> implements PaintListener {
    ...
    public enum BARTYPE {
        STEEL(ORE.IRON, 1),
        MITHRIL(ORE.MITHRIL, 2),
        ADAMANTITE(ORE.ADAMANTITE, 3),
        RUNITE(ORE.RUNITE, 4);

        private final ORE primary;
        private final int ratio; // ratio of coal to primary ore

        private BARTYPE(ORE primary, int ratio) {
        ...
        public static int getRatio(BARTYPE b) {
            return b.ratio;
        }

        public static ORE getPrimary(BARTYPE b) {
            return b.primary;
        }
    }
    ...
    private static final int ORE_INFO_VARPBIT = 547;
    private static final int BAR_INFO_VARPBIT = 545;
    ...
    public void start() {
        ...
        if(barType == null || barType != BARTYPE.MITHRIL) {
    ...
    private State state() {
        ...
        if (ctx.inventory.select().id(ORE_IDs).count() == 0 &&
                (expectedCoalCount < fullLoad * BARTYPE.getRatio(barType) || expectedPrimaryCount < fullLoad)) {
            log.info("----WITHDRAW----");
    ...
    private void updateFurnaceStatus() {
        coalCount = ctx.varpbits.varpbit(ORE_INFO_VARPBIT) & COAL_MASK;
        primaryCount = (ctx.varpbits.varpbit(ORE_INFO_VARPBIT) & MITHRIL_MASK) >> MITHRIL_SHIFT;
        barCount = (ctx.varpbits.varpbit(BAR_INFO_VARPBIT) & MITHRIL_MASK) >> MITHRIL_SHIFT;
    ...
    private int withdrawOres() {
        ...
        primaryRemaining = ctx.bank.select().id(ORE.COAL.getID()).poll().stackSize();
        coalRemaining = ctx.bank.select().id(ORE.MITHRIL.getID()).poll().stackSize();

        if (expectedCoalCount < fullLoad * BARTYPE.getRatio(barType)) {
            ...
            ctx.bank.withdraw(barType.getPrimary().getID(), Bank.Amount.ALL); // withdraw mithril
    ...
    private boolean collectBars() {
        ...
        final Component barWidget = ctx.widgets.component(BAR_WIDGET, MITHRIL_BAR_COMPONENT);
    ...
    public void setBarType(BARTYPE b) {
        barType = b;
    }
}
