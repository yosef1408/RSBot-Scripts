...
public class Walker {
    ...
    public boolean handleObstacle(Tile[] t) {
         ...
         */
        if (nextTile.matrix(ctx).reachable() || ctx.players.local().inMotion()) {
            return false;
        }
        ...
        switch (getType(go.name())) {
            case DOOR:
                switch (go.orientation()) {
                    ...
                    case 4:
                    case 6:
                        return new int[]{102, 126, 6, -220, 9, 131};
                    case 1:
                        return new int[]{63, 129, -224, -7, -10, 118};
                    case 2:
                        return new int[]{63, 129, -223, -7, 117, 250};
