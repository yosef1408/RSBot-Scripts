package Leroux.FreeWorldCooker.Methods;
...
public class Walker extends ClientAccessor {
			...
			if(curTile != null) {
				...
				Condition.wait(new Callable<Boolean>() {
					public Boolean call() throws Exception {
						return ctx.movement.distance(ctx.players.local().tile(), ctx.movement.destination()) <= Random.nextInt(2, 4);
					}
				}, 100, 10);
			} else {
				break;
	...
	public void followPath(Tile[] path, int min, int max) {
		for(int i = 0; i < path.length; i++) {
			...
			System.out.print(" Walker: curTile: " + curTile);
