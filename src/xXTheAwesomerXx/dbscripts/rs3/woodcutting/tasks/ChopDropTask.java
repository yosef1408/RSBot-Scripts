...
public class ChopDropTask extends Task<ClientContext>
		implements
			MessageListener {

	private enum State {
		BANK, CHOP, DROP, DROP_EXTRA, RUN_AWAY, RUN_TO_BANK, RUN_TO_TREE, TELEPORT
	}

	private boolean bankStartingInventory = false, runAway = false,
			started = false, dropping = false;;
			...
			public ChopDropTask(final ClientContext ctx, final int taskPosition,
			...
			case CHOP :
				...
				m.chopAndDrop(getTreeSelection(), getLocSelection(),
						getTaskTreeArea());
				break;
			case BANK :
			...
			case DROP :
				...
				CBFMultitasker.statusMessage = "Dropping";
				m.dropLogs(getTreeSelection());
				break;
			case DROP_EXTRA :
