...
public class ChopBurnTask extends Task<ClientContext>
		implements
			MessageListener {

	private enum State {
		BANK, BURN, CHOP, DROP_EXTRA, RUN_AWAY, RUN_TO_BANK, RUN_TO_TREE, TELEPORT
	}

	private boolean bankStartingInventory = false, runAway = false,
			started = false, burning = false;
	...
	public ChopBurnTask(final ClientContext ctx, final int taskPosition,
	...
	public void execute() {
		...
		switch (getState()) {
			...
			case CHOP :
				...
				m.chopAndBurn(getTreeSelection(), getLocSelection(),
						getTaskTreeArea());
				break;
			case BANK :
				CBFMultitasker.statusMessage = "Banking";
				final int[] extraAndUnstrung = m.concat(getExtraLogIds(),
						m.allUnstrungBowIds());
				bankStartingInventory = !m.bankItems(getLocSelection(),
						extraAndUnstrung);
				break;
			case BURN :
				if (ctx.backpack.select().count() <= (28 - m
						.getRandomInventInt())) {
					burning = false;
				}
				CBFMultitasker.statusMessage = "Burning";
				m.burnLogs(getTreeSelection());
				break;
			case DROP_EXTRA :
	...
	public void messaged(final MessageEvent m) {
		if (m.source().isEmpty()) {
			...
			} else if (m.text().contains("The fire catches")
					|| m.text().contains("You add a log")) {
				CBFMultitasker.totalBurnedLogs++;
				CBFMultitasker.taskBurnedLogs++;
			}
		}
	}
